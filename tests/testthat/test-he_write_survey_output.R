test_that("survey output file generates error for missing survey matrix", {
  # Create test directory
  temp_test_dir <- output_test_setup()
  # Check for error
  expect_error(
    he_write_survey_output(surveyed_output_matrix,
                           survey_output_file_name,
                           output_dir = temp_test_dir)
  )
})

test_that("survey output file generates error for missing survey file name", {
  # Create test directory
  temp_test_dir <- output_test_setup()
  # Populate environment variables
  surveyed_output_matrix <- matrix(numeric(0), ncol = 3)
  # Check for error
  expect_error(
    he_write_survey_output(surveyed_output_matrix,
                           survey_output_file_name,
...
test_that("survey output file is created in expected directory", {
  # Create test directory
  temp_test_dir <- output_test_setup()
  # Populate environment variables
  surveyed_output_matrix <- matrix(numeric(0), ncol = 3)
  ...
  expected_filepath <- file.path(temp_test_dir, survey_output_file_name)
  he_write_survey_output(surveyed_output_matrix,
                         survey_output_file_name,
                         temp_test_dir)
