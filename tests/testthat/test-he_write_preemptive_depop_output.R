test_that("preemptive depop output file generates error for missing
          preemptive depop matrix", {
...
test_that("preemptive depop output file is created in expected directory", {
  # Create test directory
  temp_test_dir <- output_test_setup()
  # Populate environment variables
  preemptive_depop_matrix <- matrix(numeric(0), ncol = 3)
  ...
  he_write_preemptive_depop_output(preemptive_depop_matrix,
                                   preemptive_depop_file_name,
                                   temp_test_dir)
