test_that("default depop parameters are stored in the environment", {
  test_environment <- rlang::new_environment()
  he_define_depop_control_params(test_environment)
...
test_that("user-defined depopn parameters stored in the environment", {
  ...
  expect_equal(
    test_environment$depop_farm_if_netpen_infected,
    test_depop_farm_if_netpen_infected
  )
})
