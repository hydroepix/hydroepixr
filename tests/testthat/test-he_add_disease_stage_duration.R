test_that("new infection stage row is appended correctly", {
  ...
  expect_equal(
    sum(tail(test_result_duration_matrix)),
    test_n_animals_to_distribute
  )
  expected_duration_matrix <- matrix(
    c(
      rep(0, 8),
...
test_that("new infection stage row is appended correctly in repeated calls", {
  withr::local_seed(100)
  test_empty_duration_matrix <- matrix(0, ncol = 8)
  test_infection_stage_distribution <-
    c(0.000001, 0.000001, 0.0001, 0.01, 0.1, 0.25, 0.339898, 0.3)
  ...
  # Second call
  test_result_duration_matrix_2 <- he_add_infection_stage_duration(
  ...
  # Numbers in new row should sum to the number of animals to distribute
  expect_equal(
    sum(tail(test_result_duration_matrix_2, n = 1)),
    test_n_animals_to_distribute
  )
  expected_duration_matrix <- matrix(
    c(
  ...
  # Third call
  test_result_duration_matrix_3 <- he_add_infection_stage_duration(
    test_result_duration_matrix_2,
    test_infection_stage_distribution,
    0
  )
  # Numbers in new row should sum to the number of animals to distribute
  expect_equal(sum(tail(test_result_duration_matrix_3, n = 1)), 0)
  expected_duration_matrix <- matrix(
    c(
      rep(0, 8),
      # Values determined by random seed:
