...
#' @param num_newly_infected he number of newly infected animals for each
#'    netpen
#'
...
he_calculate_net_change_in_disease_stage_count <-
  function(disease_stage_duration_matrices,
    ...
    animals_in_by_stage <-
      cbind(0, num_newly_infected, changes_in_disease_stage_duration,
            deparse.level = 0)
    animals_out_by_stage <-
      cbind(num_newly_infected, changes_in_disease_stage_duration, 0,
            deparse.level = 0)
