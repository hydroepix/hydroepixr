he_limit_movements <- function() {

  # If the first case is not detected, then get all clinical farms and the time
  ...
  if (!outbreakDetected) {
    ...
    tmp        <-
      (aHerd$Mortality[indexDiag] >= (aHerd$CageSize[indexDiag] * ExpectMort * MortalityIncrease1)) &
  ...
  if (outbreakDetected & gTime > gDaysUntilBaseline) {
    ...
    ExpectMort <- (gTime - SickTime) * aHerd$ExpMortality[indexDiag]
    tmp        <-
      (aHerd$Mortality[indexDiag] >= (aHerd$CageSize[indexDiag] * ExpectMort * MortalityIncrease2)) &
  ...
  if (outbreakDetected) {
    ...
    if (length(indexDiag) > 0) {
      ...
      # Adding newly diagnosed farms to the depopQueue and placing them after
      # those herds that were previously diagnosed and before non-diagnosed
      # farms tagged by interventions.
  ...
  # Check for active surveillance of farms and sampling of fish and update times
  aHerd$timeToASV[aHerd$timeToASV < gTime] <<-
    aHerd$timeToASV[aHerd$timeToASV < gTime] + round(rtriang(1, 30, 42, 60)) # 90
} ##End Of function
